function backupCourses(){
  const payload={app:'兴趣课时本',version:1,exportedAt:new Date().toISOString(),courses}
  const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json;charset=utf-8'})
  const href=URL.createObjectURL(blob),link=document.createElement('a');link.href=href;link.download=`兴趣课时本备份_${dateKey(new Date())}.json`;document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(href),1000);notify('备份文件已导出')
}
function restoreCourses(file){
  ...
  reader.onload=()=>{
    try{
      ...
      if(persist(normalized))notify(`已恢复 ${normalized.length} 门课程`)
